...
`api` 服务提供与 Web UI 所调用相同的 `POST /api/<method>` 接口，其组合不含浏览器客户端，因此程序可以通过 HTTP 创建会话、选择模型、提交提示词并读取记录： [`apps/discord-bot`](apps/discord-bot/README.md) 是基于该服务的 Discord 桥接：把 bot 令牌写入 `secrets/discord-token`，在 `.env` 中列出允许的 Discord 用户 id，然后用 `docker compose --profile discord up -d discord-bot` 启动它。
