...
- **An explicit re-pick of the route already being delegated is not adopted mid-turn.** A route change counts as external only when the delegated route differs from both the route this plugin last wrote and the previous delegation, because a route owner re-asserts a standing selection on every request and that must not read as a fresh pick. Selecting the route already being delegated is indistinguishable from that re-assertion, so the cursor stays where it is until the next turn's reset returns it to the primary.
