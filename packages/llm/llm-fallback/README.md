...
- **A route picked after the first failover applies one assembly late.** The plugin stages an external route change at `agent/request` and promotes it at the next `system-prompt/assemble`, which runs once per step, so the request that carries the pick still runs on the route the cursor already selected; the picked route serves the next step, or the next turn when the pick lands on the turn's last step. A pick landing on a turn whose primary then fails costs one further request, because the failover retry re-enters the same step without reassembling and runs on the backup. Until the session's first failover the plugin asserts no route at all, and a pick reaches the provider immediately, so this cost appears only after a failover has occurred.
- **A pick of the route already answering is never adopted; one naming a recently written route waits a turn.** A route change counts as external only when the delegated route names none of three: the primary captured at the first failover, whichever route the plugin wrote most recently, and the backups the cursor wrote during this turn or the one before. The first two are exact, because both sources of a delegation reach back one step — the durable header names the last request, and a route owner's selection is that header as of the previous step — so a delegation carrying either is the request a session with no pick produces. The turn marks are deliberately wider. The plugin does not record when within a turn each backup was written, and a promotion taken mid-turn strands the writes before it, so a cascade's earlier backups stay suppressed after every delegation source has moved off them. That width costs one turn: a pick made in the turn right after a cascade is suppressed for that turn and served on the next, so it heals itself. Only a pick of the route the plugin is currently serving is discarded for good — with the primary failing and the first backup answering, a user picking that backup goes on being served by it while the model picker reports the pick. A route off the chain, or a backup the cursor has never reached, is adopted at once. Making the answering backup pickable too would need a durable record of the selection, which is deferred.
