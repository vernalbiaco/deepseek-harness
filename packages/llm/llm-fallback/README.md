...
- **A pick naming a route already in the chain is never adopted.** A route change counts as external only when the delegated route names no route of the chain — neither the primary captured at the first failover nor any configured backup — because every route this plugin writes comes from that chain and a delegation can be that write read back, either as the durable header with no route owner mounted or as the selection a route owner captured one step earlier. The two are the same requests in the same order, so nothing computed from the delegation separates a pick of a chain route from no pick at all, and after the first failover such a pick changes nothing: the cursor keeps serving what it selects and every turn keeps re-probing the captured primary. With the primary failing and the first backup answering, a user picking either that backup or a later one is served by the first backup for as long as the agent lives, while the model picker reports the picked route. A pick outside the chain and a new agent are the escapes; a durable record of the selection itself is what would make such a pick adoptable, and is deferred.
