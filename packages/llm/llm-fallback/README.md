...
- **A route picked after the first failover applies one assembly late.** The plugin stages an external route change at `agent/request` and promotes it at the next `system-prompt/assemble`, which runs once per step, so the request that carries the pick still runs on the re-asserted primary; the picked route serves the next step, or the next turn when the pick lands on the turn's last step. Until the session's first failover the plugin asserts no route at all, and a pick reaches the provider immediately, so this cost appears only after a failover has occurred.
- **A pick naming a route already in play is recognized late, or not at all.** A route change counts as external only when the delegated route differs from both the route this plugin last wrote and the previous delegation, because a route owner re-asserts a standing selection on every request and that must not read as a fresh pick. Picking the route this plugin last wrote — after a failover, the backup the model picker reports as the live route — is indistinguishable from that write echoed back on the request that first carries it, so it is recognized on the following request and applies one assembly later than the deferral above. Re-picking the route already being delegated changes no delegation at all and is invisible to this plugin, so the cursor stays where it is until the next turn's reset returns it to the primary.
