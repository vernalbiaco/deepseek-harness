...
- **A pick naming a route this plugin has already written is never adopted.** A route change counts as external only when the delegated route names none of the routes this plugin has written — the primary captured at the first failover, a backup the cursor has reached, and the route written most recently, which a promotion can move the primary off — because a delegation can be any of them read back — either as the durable header with no route owner mounted or as the selection a route owner captured one step earlier. Those two are the same requests in the same order, so nothing computed from the delegation separates such a pick from no pick at all, and after the first failover it changes nothing: the cursor keeps serving what it selects and every turn keeps re-probing the captured primary. With the primary failing and the first backup answering, a user picking that backup is served by it for as long as the agent lives, while the model picker reports the pick. A backup further down the chain has never been requested, so picking one is adopted like any other outside choice; the escapes are that pick, a route off the chain entirely, and a new agent. Making the served backup pickable too would need a durable record of the selection, which is deferred.
