...
`backups` is required and must list at least one route with no duplicates; an empty or duplicate chain fails plugin load. `failoverCodes` is optional and, when present, must be non-empty; omitting it uses the ten codes in `src/config.ts`.
