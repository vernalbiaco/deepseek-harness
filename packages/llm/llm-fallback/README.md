...
- **A pick naming a route this plugin wrote recently is never adopted.** A route change counts as external only when the delegated route names none of the routes still within a delegation's reach. Those are three: the primary captured at the first failover, the backups the cursor wrote in this turn or the one before it, and whichever route the plugin wrote most recently. Both sources of a delegation reach back exactly one step — the durable header names the last request, and a route owner's selection is that header as of the previous step — so a delegation naming one of those three is the same request a session with no pick at all produces, and nothing computed from it separates the two. After the first failover such a pick changes nothing: the cursor keeps serving what it selects and every turn keeps re-probing the captured primary. With the primary failing and the first backup answering, a user picking that backup is served by it for as long as the agent lives, while the model picker reports the pick. Any route outside that reach is adopted like any other outside choice — a backup the cursor has never reached, a backup an older turn cascaded through, or a route off the chain — so those, and a new agent, are the escapes. Making the answering backup pickable too would need a durable record of the selection, which is deferred.
