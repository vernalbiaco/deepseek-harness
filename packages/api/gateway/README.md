...
`ctx.remote.$host` reads the fixed Host facts as plain values: `home` (undefined until the first ready frame), `isLoopback`, and `configurable`, which Connection derives from the page authority and the Host's configuration authority. It is not a store — no subscription, no generation counter — so a consumer that must react to reconnection listens for `connection/reset` instead of polling it.
