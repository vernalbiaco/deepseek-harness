...
`ctx.remote.$host` 以普通值读取固定的 Host 事实：`home`（首个 ready 帧之前为 undefined）、`isLoopback` 与 `configurable`，后者由 Connection 根据页面 authority 与 Host 的配置 authority 推导。它不是存储——没有订阅、没有代次计数——所以需要响应重连的消费方去监听 `connection/reset`，而不是轮询它。
