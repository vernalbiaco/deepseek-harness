...
不是 subagent 子 agent 的符合条件的 agent 会提出一个问题，列出每个未决定的服务器及其传输方式、命令与参数或 URL，以及每个引用的凭据是 `set` 还是 `missing`。每个工作区路径与未决定服务器名称及指纹的集合只存在一个待定问题；未决定集合相同的会话等待其回答。如果提问的 agent 回答 Allow this session 或被释放，或者提问界面中止了该问题，每个等待中的会话会提出自己的问题。
