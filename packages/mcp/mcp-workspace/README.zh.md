...
工作区声明的 MCP 服务器：从每个会话的规范 cwd 读取 `.mcp.json`，仅在用户决定存储于工作区之外后才连接所声明的服务器，在使用同一工作区服务器的会话之间共享一个受监管连接，并以 [`dsh-mcp-client`](../mcp-client/README.md) 的名称 `mcp__<serverName>__<rawName>` 将该服务器的工具注册到每个符合条件的 agent（智能体）自己的工具层。
...
不是 subagent 子 agent 的符合条件的 agent 每个工作区只提出一个问题，列出每个未决定的服务器及其传输方式、命令与参数或 URL，以及每个引用的凭据是 `set` 还是 `missing`。同一工作区中的其他会话等待该回答。
...
服务器每次挂接到 agent 时都会重新检查已存储的决定：创建 agent 时检查一次，该 agent 读取 `.mcp.json` 后再检查一次，异步挂接之前立即再检查一次。`deny`、指纹改变、没有 Allow this session 许可时条目缺失，或信任文件无法读取，都会从该 agent 撤回该服务器并释放插件持有的预连接引用。无法读取或无效的信任文件不准入任何服务器，包括本会话已允许的服务器。
...
- **不监视 `.mcp.json`**：编辑对该工作区中的下一个会话生效；指纹改变时会再次询问，而先前的连接会保留到其最后一个引用被释放。
