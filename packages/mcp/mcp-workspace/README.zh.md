...
| `preconnectTimeoutMs` | 否 | 激活等待预连接服务器首次连接尝试的最长时间，单位毫秒（默认 10000） |
| `toolCallTimeoutMs` | 否 | 每个工作区服务器单次工具调用的超时，单位毫秒（默认 60000） |
...
不是 subagent 子 agent 的符合条件的 agent 会提出一个问题，列出每个未决定的服务器及其传输方式、命令与参数或 URL，以及每个引用的凭据是 `set` 还是 `missing`。每个工作区路径与未决定指纹集合只存在一个待定问题；未决定集合相同的会话等待其回答。如果提问的 agent 回答 Allow this session 或被释放，每个等待中的会话会提出自己的问题。
...
服务器每次挂接到 agent 时都会重新检查已存储的决定：创建 agent 时检查一次，该 agent 读取 `.mcp.json` 后再检查一次，异步挂接之前立即再检查一次。已存储的 `deny`、没有 Allow this session 许可时条目缺失、已存储条目的指纹与 `.mcp.json` 条目不再匹配，或信任文件无法读取，都会从该 agent 撤回该服务器并释放插件持有的预连接引用。在 `.mcp.json` 中移除或更改该条目，会从之后读取该文件的每个 agent 撤回该服务器，但插件持有的预连接引用及其连接会保留到插件被释放。无法读取或无效的信任文件不准入任何服务器，包括本会话已允许的服务器。
...
- **不监视 `.mcp.json`**：编辑对该工作区中的下一个会话生效，指纹改变时会再次询问。已移除或已更改条目的预连接连接会保持打开，直到插件被释放，因为只有对同一条目的已存储决定重新检查才会释放插件持有的预连接引用。
