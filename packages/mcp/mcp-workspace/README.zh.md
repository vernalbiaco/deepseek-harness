...
在其工作区服务器已发布工具之后创建的 agent，会在第一次模型请求之前获得这些工具。会话期间准入的服务器，或在 agent 创建之后才完成连接的服务器，会在后续步骤注册其工具。传输丢失时，最后一组工具保持注册，调用会失败，并按 `reconnect` 策略重连；尝试预算耗尽时，工具会从每个已挂接的 agent 移除，下一个挂接的会话会启动新连接。注册之前，如果存在同名全局工具，每个工作区与服务器只记录一次警告。符合条件的 agent 的异步接纳流程结束时（包括未准入任何服务器的情况），插件会发出内存事件 `mcp-workspace/binding-settled`，携带 `{ agent }`；它绝不会进入模型或会话日志。
...
- **批准覆盖的是 `.mcp.json` 条目，而不是条目运行的内容**：指纹只覆盖条目文本，因此被修改的工作区脚本（例如 `./mcp/server.js`）、被修改的 `package.json` 脚本，或 `npx -y` 解析到的新包版本，都会在现有批准下运行，而不会再次询问。
- **启动期间的关闭可能等待预连接**：激活在 `apply` 内等待进程 cwd 的预连接服务器，因此在此期间请求的关闭，在已批准的服务器始终不响应时最多等待 `preconnectTimeoutMs`。
- **服务器可能短暂运行两份**：在某服务器最后一个引用仍在关闭时再次获取它，会在正在退出的连接旁启动新连接；因失败尝试始终未报告关闭而停止的连接，会在下一次获取时被替换，即使旧子进程仍在运行。持有独占资源（例如锁文件或端口）的服务器可能看到两个实例。
- **每个应用只能有一个 `mcp-workspace` 行**：不变量伴随插件按根上下文查找每个插件的连接池，因此同一应用中的第二行会替换第一行的条目。
- **Allow for this workspace 之后问题可能立即重复**：在另一个会话的 Allow for this workspace 被记录之前刚刚查过已存储决定的会话，会再次提出同一问题；回答它会记录相同的决定。
- **删除的信任条目会在不询问的情况下撤回**：信任条目在 agent 的同步挂接与异步重新检查之间被删除时，该 agent 会失去该服务器而不被询问；该工作区中的下一个会话会询问。
- **deny 不会立即停止正在连接的预连接**：在预连接服务器仍在连接时写入的 `deny`，会让该连接一直运行，直到该工作区创建下一个 agent 或插件被释放；在此期间没有 agent 会挂接它。
