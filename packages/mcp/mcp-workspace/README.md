...
Every attachment of a server to an agent rechecks the stored decision: when an agent is created, once more after that agent's `.mcp.json` read, and immediately before an asynchronous attach. A stored `deny`, even for an agent holding Allow this session; a missing entry, or a stored entry whose fingerprint no longer matches the `.mcp.json` entry, for an agent without Allow this session; or an unreadable trust file withdraws the server from that agent and releases the plugin-held preconnect reference. Each of those attachment points also reads `.mcp.json`: an entry the file no longer declares, and every entry when the file is missing or cannot be parsed, is withdrawn from that agent and its plugin-held preconnect reference released, so the connection closes once no agent holds it. An unreadable or invalid trust file admits nothing, including servers allowed for this session.
...
- **`.mcp.json` is not watched** — an edit takes effect when the next agent in that workspace is created, and a changed fingerprint asks again. Running agents keep the tools of a removed or changed entry, and its connection stays open until they are disposed. An agent created while the file is missing or cannot be parsed, such as in the middle of an edit, withdraws every server of that workspace and closes each connection no other agent holds.
...
- **Shutdown during startup can wait for preconnect** — activation waits for the process cwd's preconnected servers inside `apply`, so a shutdown requested meanwhile waits up to `preconnectTimeoutMs` when an approved server never answers.
- **A server can briefly run twice** — acquiring a server again while its last reference is still closing starts a new connection beside the exiting one, and a connection stopped because a failed attempt never reported closing is replaced on the next acquire even if that old child is still running; a server that holds an exclusive resource, such as a lock file or port, can see two instances.
- **One `mcp-workspace` row per app** — the invariant companion finds each plugin's pool by root context, so a second row in the same app replaces the first row's entry.
...
- **A deleted trust entry withdraws without asking** — an agent whose entry is deleted between its synchronous attachment and its asynchronous recheck loses that server without a question; the next session in that workspace asks.
- **A deny does not stop a connecting preconnect at once** — a `deny` written while a preconnected server is still connecting leaves that connection running until the next agent in that workspace is created or the plugin is disposed; no agent attaches it meanwhile.
