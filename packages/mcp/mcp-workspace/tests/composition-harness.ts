 ...
 * @param options - model script, server env, and Harness-home `.env` values.
...
export async function bootComposition(options: CompositionOptions): Promise<Composition> {
  ...
  for (const dir of [home, workspace, pidDir]) await mkdir(dir)
  ...
  const ctx = await boot(BIN, configPath, [], (bootCtx) => {
    ...
    Object.assign(bootCtx.loader.builtins, {
      ...
      'mcp-workspace-test-session-projection': SessionProjectionRegistry,
...
/**
 * Flush an agent's session and return its events in log order.
 * @param ctx - the composition context.
 * @param agent - the agent whose session events to return.
 * @returns the session events in log order.
 */
export async function sessionEvents(ctx: Context, agent: Agent): Promise<readonly SessionEvent[]> {
