...
Ship `apps/discord-bot`, a private app that is an ordinary client of the `api` service. It calls `session.create`, `session.prompt`, and `session.cancel` over `POST /api/<method>`, consumes `/api/events.mux` over WebSocket, and answers `approval/requested` and `question/requested` frames through `/api/respond`. Sessions it creates therefore appear in the Web UI and share the deployment's model default, presets, and tools. They are filed under one workspace: an existing workspace titled `Discord` is adopted by title, because the user may have created it in the Web UI with a directory of their own; otherwise, since a workspace is one canonical directory, the bot registers a configured directory (default `/workspaces/Discord`, created through `host.createDirectory` when absent) under that title. Every session is created with that `workspaceId`.
