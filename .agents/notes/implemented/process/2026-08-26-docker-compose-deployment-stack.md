...
The public hostname's only authentication is Cloudflare Access, configured outside this repository. Neither the compose files nor the fence nor the gateway checks that it is present, so the deployment's safety rests on a control none of its own gates can see.
