...
`harness.ernestojpamajr.com` adds a third path to the same service, from the internet. A cloudflared ingress rule forwards it to Traefik's `web` entrypoint on the loopback, which the tunnel reaches from inside the host, so the loopback bind stops nothing once that rule exists. That rule is managed in Cloudflare, not on this host: the tunnel takes a remote configuration, so a local `config.yml` is ignored and the ingress can be neither read nor changed from a checkout. The name is declared to the trust fence rather than rewritten to an internal one at the tunnel: the fence requires `Origin` to equal `Host` whenever a browser sends an `Origin`, and over HTTPS it always does, so an `httpHostHeader` rewrite would 403 every `/api` call. Cloudflare terminates TLS at the edge and the origin hop is plain HTTP on the loopback, so the tunnel reaches Traefik's `web` entrypoint. The name also carries a `websecure` router for a caller that reaches Traefik directly, and the locally minted certificate names it in `subjectAltName` for that path.

### Routing model traffic through a gateway

[`docker-compose.omni.yml`](../../../../docker-compose.omni.yml) runs OmniRoute, an OpenAI-compatible gateway, as its own Compose project, and [`docker-compose.omni-wire.yml`](../../../../docker-compose.omni-wire.yml) joins the three `dsh` services to its network so their model traffic terminates there instead of at the public API. The gateway is a separate project for the reason the infra project is: it outlives any one harness run and serves host-side tools, so `make docker-down` leaves it running. Its data volume is `external` because that volume holds the auto-generated signing secrets every minted key is issued against, and a fresh volume would invalidate all of them. The gateway carries its own Traefik router labels and joins `hybrid_public_network` as external, so Traefik serves it as `omni.ernestojpamajr.com` and `omni.localhost` on the entrypoints the harness services use. Traefik discovery is therefore a dependency of the gateway project and not only of the harness services: `make docker-omni` starts the infra project first, and `make docker-infra-down` fails while the gateway holds an endpoint on that network. The `websecure` router presents the locally minted certificate, so both names appear in its `subjectAltName`.
...
The public hostname's only authentication is Cloudflare Access, configured outside this repository. Neither the compose files nor the fence nor the gateway checks that it is present, so the deployment's safety rests on a control none of its own gates can see. The override leans on it further: `--configuration-authority trusted-host` admits every trusted name — the public one and the RavenStack and localhost names alike, since the fence is per deployment, not per hostname — to Settings, credentials, and preset management, which the default fence keeps loopback-only ([decision](../architecture/2026-07-28-api-browser-trust-boundary.md)). Without Access in front, that is an unauthenticated configuration plane on the internet; with it, the browser on the public name persists settings while the Host's native document and dialog methods stay refused.
