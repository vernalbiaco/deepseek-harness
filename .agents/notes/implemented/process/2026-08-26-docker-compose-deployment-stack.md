...
`harness.ernestojpamajr.com` adds a third path to the same service, from the internet. A cloudflared ingress rule on the host forwards it to Traefik's `web` entrypoint on the loopback, which the tunnel reaches from inside the host, so the loopback bind stops nothing once that rule exists. The name is declared to the trust fence rather than rewritten to an internal one at the tunnel: the fence requires `Origin` to equal `Host` whenever a browser sends an `Origin`, and over HTTPS it always does, so an `httpHostHeader` rewrite would 403 every `/api` call. Cloudflare terminates TLS at the edge and the origin hop is plain HTTP on loopback, so the locally minted certificate neither covers that name nor needs to.
