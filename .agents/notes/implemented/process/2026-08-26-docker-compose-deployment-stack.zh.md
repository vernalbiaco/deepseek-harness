...
网关只接受由其自身仪表盘签发的密钥，公开 API 只接受真实的 DeepSeek 密钥，因此密钥随模式而变，而 `.env` 不变。`.env` 中 `DEEPSEEK_API_KEY` 供直连模式使用，`OMNIROUTE_API_KEY` 供网关使用，覆盖文件在已接入的服务内部把后者映射为前者，这一点成立是因为后加载的 Compose 文件中 `environment:` 的同名条目会覆盖基础文件中的条目。两个网关变量都以 `:?` 声明为必填，因此缺失或为空都会在 `config` 阶段指名该变量并失败，而不是等到第一次模型请求。
...
**把 GitHub 令牌作为环境变量转发。** 这只需一行 Compose，而且与网关叠加层传递模型密钥的方式一致。但模型密钥由 harness 进程自身读取，而 GitHub 令牌必须到达代理派生出的子进程 git，子进程接缝则会按设计从该子进程的环境中剥离形似凭据的名称。`docker compose exec` 检查会通过，而代理自己的推送会失败。

**挂载宿主机的 `~/.ssh`，让现有远端原样可用。** 这样远端无需重写，但该目录持有宿主用户的每一把密钥，其中大多与这些仓库无关，而且每一把都会被未认证的代理读取。一个由 GitHub 自身权限限定范围的令牌文件才是更窄的授权。
...
**把整份 `.env` 作为 `env_file` 加载进每个服务。** 这是最初的安排，每个服务只需一行。但它也把文件里的每个变量——包括网关仪表盘密码——放进了每个服务的启动环境，而凭据存储把启动环境视为只读的运维意图：任何派生凭据引用与被转发名称相同的路由都完全无法从 Models 页面配置。因此直连模式什么都不注入，改由 `dsh` 自行读取该文件，让凭据存储保持权威；只有网关覆盖文件注入，且仅注入网关模式必须掌控的密钥。
