...
每次挂接都会重新检查已保存的决定：`agent/created` 中的同步挂接、该 agent 读取 `.mcp.json` 之后的检查，以及异步挂接之前的即时检查。已保存的 `deny`、缺少条目或指纹变化会从该 agent 撤下该服务器，并释放插件持有的预连接引用，除非该 agent 对该服务器持有“仅本会话允许”：无论已保存的决定是什么，`admits()` 都接受会话内允许。信任文件无法读取或无效时，会撤下所有服务器，包括会话内允许的服务器。
