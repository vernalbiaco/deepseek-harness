...
- 批准固定的是 `.mcp.json` 条目文本，而不是该条目运行的程序、脚本或包：对于 `{"command": "node", "args": ["./mcp/server.js"]}`，改写 `server.js` 的提交、被修改的 `package.json` 脚本，或 `npx -y` 解析到的新版本，都会在现有批准下运行，而不会再次提问。
...
- 在某个键的最后一个租约仍在关闭时获取该键，会在正在退出的连接旁启动新连接，因此持有独占资源（例如锁文件或端口）的服务器可能短暂看到两个实例。
- 因失败的世代未在监管器关闭超时内报告关闭而停止的连接，会在下一次获取时重启；如果旧子进程仍在运行，新旧子进程可能重叠。
- `activePools` 按根上下文作为键，因此一个应用最多挂载一个 `mcp-workspace` 行；第二行会为不变量伴随插件替换第一行的连接池。
- 某个 agent 的信任查找恰好在另一个 agent 的“对此工作区允许”被记录之前运行、而其等待在该问题关闭之后才开始时，会再次提出同一问题。
- 信任条目在 agent 的同步挂接与异步重新检查之间被删除时，该 agent 会失去该服务器且不被提问；该工作区中下一个创建的 agent 会提问。
- 预连接服务器仍在连接时写入的 `deny` 只会在下一次流程中被撤销：在此期间没有 agent 会挂接该服务器，但其连接会一直运行，直到该工作区创建下一个 agent 或插件被释放。
