...
- **ACP snapshots:** `examples/acp-agent` scenario `workspace-mcp` seeds a saved `allow` for a generated `.mcp.json` that names `tests/fixtures/mcp-echo-server.mjs` and pins the request header and tool schemas with `mcp__fixture__echo` in the first step. Scenario `workspace-mcp-unapproved` has no decision; its request header equals the default class pin, and `acp.snapshot.ts` asserts the stderr line `mcp-workspace(fixture): not approved; no question UI is available`. That warning comes from the asynchronous admission pass, so the scenario's `input.json` waits for `.dsh-snapshot-workspace-mcp-settled`, which the snapshot-only `tests/fixtures/workspace-mcp-settlement-marker.ts` row writes on `mcp-workspace/binding-settled`, before the harness closes stdin. The snapshot-only `tests/fixtures/stderr-log-exporter.ts` row delivers that line, because Cordis exporters without `levels` drop `warn`.
