...
Only a route differing from both signals is a user selection or settings change; the plugin stages it and the next assembly promotes it to primary, restarting the chain from it. The cost is that an explicit re-pick of the route already being delegated is indistinguishable from the standing assertion and waits for the next turn's reset.
...
A separately published `./invariant` companion checks that every `llm/fallback` names the currently open turn and step — the step stays open through a mid-step cascade, so a closed one is the error — that its `from` route matches the durable header in force, that its `to` route matches the next `request/header` appended before that step closes, and that the cursor is a positive integer one greater than the previous record's within the same step. The forward header check is retired at the owning step's `step/end`, because a retry whose route already equals the header in force appends no header of its own and leaves that record unconfirmed. The chain length is not durable, so the companion does not re-check the bound `advance()` enforces by construction; doing so would mean logging the chain length on every record.
