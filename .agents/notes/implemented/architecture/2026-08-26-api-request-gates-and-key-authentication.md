...
Both WebSocket upgrade handlers ([`index.ts`](../../../../packages/client/connection/src/index.ts)) call `authorizeApiRequest` after the trust fence, and an unadmitted verdict rejects the upgrade at the socket with `403` — an upgrade has no response body to carry a gate's own status.
...
`name` is the audit label and must be unique. `secret` is a [credential reference](../../../../packages/credentials/credentials/README.md). An empty `keys` list, a duplicate `name`, and a malformed reference each fail at load rather than at the first request, and a reference that resolves to nothing is skipped rather than matched against an absent secret — a caller presenting nothing therefore ends at the same `401` as an unrecognized secret.
