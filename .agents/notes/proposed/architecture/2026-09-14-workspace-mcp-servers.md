...
Each server's decision is looked up by canonical path, server name, and fingerprint. A matching `allow` connects; a matching `deny` skips without asking; a missing entry or a changed fingerprint needs a decision. The fingerprint excludes resolved credential values, so rotating a token does not re-ask, while changing a command, argument, URL, or header template does.
