...
To reach the Web UI from another device, put this host on a private network rather than publishing a port. With Tailscale, `tailscale serve --bg 3080` proxies the tailnet name to the loopback port and terminates TLS with a publicly trusted certificate, so the origin is a secure context and no port is exposed; set `DSH_TAILNET_HOST` in `.env` to that `*.ts.net` name so the trust fence accepts it. `tailscale funnel` publishes to the whole internet instead and is the wrong command here. Whoever reaches the Web UI can run code in the container and read the mounted Claude and Codex credentials, so the private network — not the fence — is what limits access.
