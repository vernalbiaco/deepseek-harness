...
The `api` service serves the same `POST /api/<method>` surface the Web UI calls, composed without a browser client.
...
Every `/api` call and the `/api/remote.mux` WebSocket first need a browser-session cookie, minted by opening the `?token=` URL that `dsh web` prints at startup on the hostname the browser uses. [`@deepseek-ai/dsh-api-key-auth`](packages/api/key-auth/README.md) gates run only after that check, so a program holding a bearer key alone is refused with `401`, and the `api` profile, which composes no web app and prints no token URL, admits no caller.
...
Each mode's targets carry their own Compose file set. Pointing a target that has no mode variant — `make docker-omni-check`, say — at services already running in gateway or Traefik mode means passing that set in as `COMPOSE_FILE`. `make docker-patch-plugins` and `make docker-check-plugins` take no file set: they act on the running containers, found by their Compose labels.
