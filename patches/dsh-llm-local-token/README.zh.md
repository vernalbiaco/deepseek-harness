...
针对第三方插件 [`dsh-llm-local-token`](https://github.com/tianxia--/dsh-llm-local-token) 的本地修复：两个被替换的模块，以及一处就地编辑。已发布的 **1.3.2** 与 **1.5.1** 附带的这两个被替换模块逐字节相同，因此同一份打过补丁的副本对两个版本都适用；而就地编辑所改动的 `index.js` 在两个版本之间并不相同。
...
该插件使用本机 Claude Code 与 Codex CLI 已持有的 OAuth 令牌来服务 LLM 调用，从而让个人订阅成为一条模型路由。两处缺陷会在 Linux 上阻断或危及这一点，第三处则让已经注册成功的路由无法出现在模型选择器中。
...
**`index.js` —— 该插件服务的每条路由都在模型选择器中加载失败。** `profileOf` 构建适配器读取的 pi-ai Provider Profile。`packages/llm/llm-pi-ai/src/config.ts` 中的 `ResolvedPiAiProviderProfile` 要求带有 `modelErrors`，而截至 1.5.1 的各发布版本都没有该字段，于是 `PiAiAdapter.modelOf` 会为每条路由抛出 `Cannot read properties of undefined (reading 'get')`，选择器把它们逐条列为加载失败的分组。其余环节毫无察觉：用量路由不读取任何 Profile，因此配额徽标自始至终都照常报告两个 Provider。该编辑补上这个缺失的字段。
...
第三处修复并非此目录中的文件，而是 `docker/plugin-patches.sh` 插入的一行：`index.js` 在各发布版本之间并不相同，存一份副本会把补丁钉死在其中某一个版本上。
...
就地编辑与复制一并执行；一旦某个发布版本自己声明了该字段，它便是空操作。它锚定在相邻的那个必填字段上——截至目前打过补丁的每个发布版本都恰好写出该字段一次；若某个发布版本写出的次数不是一次，运行会就此停止，而不去猜测新字段该放在哪里。

在对该包执行任何 `dsh plugin ... add`、更新或重新安装之后都需要重新应用：pnpm 会替换整个包目录，因此打过补丁的模块会被静默覆盖，Claude 路由随之再次消失。可用 `make docker-check-plugins` 验证，它会列出已注册的路由以及该 Profile 字段是否就位——健康的 `web` 或 `api` 服务会同时报告 `openai-codex` 与 `anthropic`，随后是 `modelErrors declared`。仅凭路由列表无法区分模型选择器是正常还是已损坏。

## 上游提交

三处修复都应回到上游 <https://github.com/tianxia--/dsh-llm-local-token>。`.patch` 文件适用于发布 tarball 中的 `lib/`；在该仓库源码中做出等价改动即可让本目录退场。
